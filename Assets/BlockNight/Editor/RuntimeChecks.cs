...
public static class RuntimeChecks {
 ...
 game.model.GrantCharge(PickupKind.时间暂停);Assert(game.ActivateSlow(),"K activates slow");Frames(60);game.model.GrantCharge(PickupKind.时间回溯);Assert(game.Rewind(),"L rewinds from idle");Frames(60);Assert(game.mode==Mode.Playing&&!game.model.HasCharge(PickupKind.时间回溯),"rewind completion");
 game.model.reward=.001f;Frames(1);Assert(game.mode==Mode.Draft,"draft opens");Assert(game.hud.cardTexts[0].text.StartsWith("U /")&&game.hud.cardTexts[1].text.StartsWith("I /")&&game.hud.cardTexts[2].text.StartsWith("O /"),"UIO draft hints");game.Choose(1);Assert(game.mode==Mode.Playing,"I selects middle card");game.model.reward=.001f;Frames(1);Assert(game.mode==Mode.Draft,"draft reopens");game.hud.cards[0].onClick.Invoke();Assert(game.mode==Mode.Playing,"card binding");game.Pause();float frozen=game.model.elapsed;Frames(60);Assert(game.mode==Mode.Paused&&game.model.elapsed==frozen,"pause freezes");game.Pause();game.model.hit=true;Frames(1);Assert(game.mode==Mode.Dying,"fatal grace");Frames(150);Assert(game.mode==Mode.Dead,"death");game.Begin(false);Assert(game.model.score==0&&game.mode==Mode.Playing,"retry");
