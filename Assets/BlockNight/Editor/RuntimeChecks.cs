...
public static class RuntimeChecks {
 ...
 game.model.reward=.001f;Frames(1);Assert(game.mode==Mode.Draft,"draft opens");game.hud.cards[0].onClick.Invoke();Assert(game.mode==Mode.Playing,"card binding");game.Pause();float frozen=game.model.elapsed;Frames(60);Assert(game.mode==Mode.Paused&&game.model.elapsed==frozen,"pause freezes");game.Pause();game.model.hit=true;Frames(1);Assert(game.mode==Mode.Dying,"fatal grace");Frames(50);Assert(game.mode==Mode.Dead,"death");game.Begin(false);Assert(game.model.score==0&&game.mode==Mode.Playing,"retry");
 game.Begin(true);game.MoveOrTurn(Vector2.right);Assert(game.tutorialStep==1&&!game.model.moving,"turn lesson");game.MoveOrTurn(Vector2.right);Frames(20);Assert(game.tutorialStep==2,"walk lesson");Frames(90);game.MoveOrTurn(Vector2.up);game.StartSlash();Frames(60);Assert(game.tutorialStep==3&&game.model.kills==3,"slash lesson");game.ActivateSlow();Frames(180);Assert(game.tutorialStep==5,"deferred release lesson");Assert(game.Rewind(),"tutorial rewind");Frames(60);Assert(game.tutorialStep==6,"tutorial completed");game.hud.skipButton.onClick.Invoke();Assert(!game.model.tutorial,"skip binding");
