 ...
 public static class GridCombatChecks {
  ...
  public static string Run(){assertions=0;var balance=ScriptableObject.CreateInstance<Balance>();var schedule=ScriptableObject.CreateInstance<SpawnSchedule>();schedule.phases=new SpawnPhase[0];var definition=ScriptableObject.CreateInstance<EnemyDefinition>();
   try {
    ...
    m=new CombatModel(balance,schedule){grace=20};m.Spawn(1,CombatModel.Center(new Vector2Int(2,2)));m.foes[0].age=3;float before=m.foes[0].timer;Check(m.GrantCharge(PickupKind.时间暂停)&&m.Slow(),"pickup charge enables time fold");m.Step(.05f);Check(Mathf.Abs(m.foes[0].timer-(before-.05f*.18f))<.0001f,"time fold slows enemy rules");
