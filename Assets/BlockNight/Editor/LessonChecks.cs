...
namespace BlockNight.Editor {
 public static class LessonChecks {
  ...
  static void SteerSlash(){d.MoveOrTurn(Vector2.up);Check(d.StartSlash(),"manual slash starts");int guard=0;while(!(d.model.cell.y==3&&d.model.travel>.45f)&&guard++<100)d.Tick(.02f);Check(guard<100&&d.model.kills==0,"turn window is before the first enemy");d.MoveOrTurn(Vector2.right);Check(d.model.queuedDirection==Vector2.right,"mid-dash direction is queued");}
  ...
  public static void SlowFixture(){Get();d.Begin(true);Dialogue();d.MoveOrTurn(Vector2.right);d.Tick(.02f);Dialogue();d.MoveOrTurn(Vector2.right);Frames(100);SteerSlash();Until(LessonStage.ObserveNormal);Dialogue();Until(LessonStage.SlowKey);d.ActivateSlow();Frames(15);}
