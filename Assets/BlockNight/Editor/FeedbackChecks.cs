...
namespace BlockNight.Editor {
 public static class FeedbackChecks {
  ...
  public static void Run(){var d=Game();d.Begin(false);var f=d.feedback;var m=d.model;
   ...
   d.Begin(false);m=d.model;m.tutorial=true;m.Spawn(0,CombatModel.Center(new Vector2Int(2,5)));m.foes[0].age=3;m.foes[0].attackWindup=true;m.foes[0].attackTarget=new Vector2Int(3,5);m.foes[0].timer=m.RulesFor(0).attackWarning;d.arena.Render(m);var aim=d.arena.FoeAim(0);float start=Vector3.Distance(aim.GetPosition(0),aim.GetPosition(1));m.foes[0].timer=0;d.arena.Render(m);aim=d.arena.FoeAim(0);float end=Vector3.Distance(aim.GetPosition(0),aim.GetPosition(1));Check(end>start*4.9f,"warning grows fivefold");
   d.Begin(false);m=d.model;m.tutorial=true;m.Spawn(0,CombatModel.Center(new Vector2Int(2,5)));m.foes[0].age=3;d.arena.Render(m);m.foes[0].pos=CombatModel.Center(new Vector2Int(3,5));d.arena.Render(m);Check(DOTween.IsTweening(d.arena.FoeTransform(0))&&Vector3.Distance(d.arena.FoeTransform(0).position,d.arena.World(m.foes[0].pos))>.01f,"enemy grid movement uses DOTween instead of snapping");
   ...
   d.Begin(false);m=d.model;m.tutorial=true;int k=f.slowReadyCount,l=f.rewindReadyCount;m.GrantCharge(PickupKind.时间暂停);m.GrantCharge(PickupKind.时间回溯);Check(f.slowReadyCount==k+1&&f.rewindReadyCount==l+1,"each pickup flash fires");for(int i=0;i<30;i++)d.Tick(.02f);Check(f.slowReadyCount==k+1&&f.rewindReadyCount==l+1&&!f.readyFlash.enabled,"no repeated ready flash");
