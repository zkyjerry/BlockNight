...
namespace BlockNight.Editor {
 public static class WaveRadiusChecks {
  ...
  public static void Run(){
   ...
   m.StartUpgradeWave(.5f,3);m.Step(.1f);var saved=m.Capture();m.Step(.1f);m.Restore(saved);Check(m.upgradeWaveMaxRadius==3&&m.upgradeWaveRadius==saved.upgradeWaveRadius,"rewind retains configured radius");m.GrantCharge(PickupKind.时间回溯);d.Rewind();
