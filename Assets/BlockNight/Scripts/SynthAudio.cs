...
public class SynthAudio:MonoBehaviour {
 public AudioSource music,effects;AudioClip[] cues;float beat;int step;bool muted;
 void Awake(){cues=new AudioClip[8];for(int i=0;i<cues.Length;i++)cues[i]=Tone(i);}
 ...
 void OnDestroy(){foreach(var c in cues)if(c)Destroy(c);}
}
}
