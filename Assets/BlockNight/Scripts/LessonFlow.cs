...
namespace BlockNight {
 ...
 public class LessonFlow:MonoBehaviour {
  ...
  public void AfterTick(float dt){
   ...
   else if((stage==LessonStage.Impact||stage==LessonStage.RewindKill)&&director.mode==Mode.Dying){stage=LessonStage.RewindKey;m.GrantCharge(PickupKind.时间回溯);Say(new[]{"你被击中了。\n世界停在受击瞬间，红色边缘和镜头聚焦提示危险。","按 L 回溯到受击之前。\n这一次，你可以做出不同的选择。"},()=>Prompt("按 L / 改写这次死亡\n回到攻击发生之前。"));}
  ...
  void Prepare(Vector2Int playerCell,EnemyDefinition definition,Vector2Int foeCell){var m=director.model;Array.Clear(m.foes,0,m.foes.Length);Array.Clear(m.shots,0,m.shots.Length);Array.Clear(m.pickups,0,m.pickups.Length);m.chargeCount=0;m.hit=false;m.draft=false;m.moving=m.dashing=false;m.player=CombatModel.Center(playerCell);m.cell=m.target=playerCell;m.travel=0;m.direction=Vector2.up;m.queuedDirection=Vector2.zero;m.slow=m.slowCD=m.rewindCD=m.grace=0;m.tutorialEnemyActions=true;m.enemyDefinitions=(EnemyDefinition[])director.enemyDefinitions.Clone();m.enemyDefinitions[0]=definition;m.Spawn(0,CombatModel.Center(foeCell));m.foes[0].age=definition.rules.spawnWarning;m.foes[0].timer=definition.rules.attackInterval;director.ClearHistory();director.arena.ClearEffects();director.feedback.ResetEffects();director.mode=Mode.Playing;startingKills=m.kills;stageTime=0;}
  void SetupSlow(){Prepare(new Vector2Int(3,1),slowEnemy,new Vector2Int(3,4));var m=director.model;m.foes[0].attackWindup=true;m.foes[0].attackTarget=new Vector2Int(4,4);m.foes[0].timer=slowEnemy.rules.attackWarning;m.GrantCharge(PickupKind.时间暂停);stage=LessonStage.ObserveNormal;Prompt("先观察红色预警框变大的速度。\n它填满时，敌人会撞向右边的格子。");}
