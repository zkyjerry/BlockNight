...
public class GameHUD:MonoBehaviour {
 ...
 public static readonly string[] Names={"超驱","轻步","时井","快充","深忆","动量","复利","静域","残像","盾弧","积分补给"};
 public static readonly string[] Descriptions={"冲刺速度 +12%\n更快穿越战场。","走格速度 +20%\n更快调整下一次斩击位置。","慢时间持续 +0.6 秒\n延长爆发前的悬念。","慢时间冷却 -1.5 秒\n更频繁地折叠时间。","回溯冷却 -2 秒\n让下一次机会更早到来。","击杀推进强化 +0.35 秒\n以战养战，加速成长。","击杀积分 +25%\n每一次斩击都更值钱。","慢速世界倍率 -4%\n让弹幕进一步静止。","回溯后保护 +0.25 秒\n留出改写选择的空间。","冲刺时展开前方弧形护盾\n抵挡一次正面攻击；破盾后冷却 8 秒。","积分 +2,500\n所有系统已强化至满级。"};
 public void Bind(GameDirector d){director=d;if(startButton)startButton.onClick.AddListener(()=>d.Begin(false));if(learnButton)learnButton.onClick.AddListener(()=>d.Begin(true));resumeButton.onClick.AddListener(d.Pause);if(retryButton)retryButton.onClick.AddListener(()=>d.Begin(false));if(homeButton)homeButton.onClick.AddListener(d.Home);skipButton.onClick.AddListener(()=>d.Begin(false));for(int i=0;i<3;i++){int k=i;cards[i].onClick.AddListener(()=>d.Choose(k));}}
 public void SetOffers(int[] ids,int[] levels){for(int i=0;i<3;i++){int id=ids[i];cardTexts[i].text="0"+(i+1)+" / 强化\n\n"+Names[id]+"\n\n"+Descriptions[id]+"\n\n"+(id<10?"等级 "+(levels[id]+1)+" / "+CombatModel.MaxLevel(id):"补给");}}
 ...
 public void Render(GameDirector d,bool historyReady){var m=d.model;readouts.SetActive(d.mode!=Mode.Title);if(titlePanel)titlePanel.SetActive(d.mode==Mode.Title);pausePanel.SetActive(d.mode==Mode.Paused);draftPanel.SetActive(d.mode==Mode.Draft);if(deathPanel)deathPanel.SetActive(d.mode==Mode.Dead);tutorialPanel.SetActive(m.tutorial&&d.mode!=Mode.Title);
